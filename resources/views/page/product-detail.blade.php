                        ...
                        <div id='carousel-custom' class='carousel slide' data-ride='carousel'>
                            <div class='carousel-outer'>
                                ...
                                <div class='carousel-inner '>
                                    @foreach($imageProducts as $key => $image)
                                        <div class='item {{ $key == 0 ? 'active' : '' }}'>
                                            <img src='{{ asset($image->image_url) }}' alt='' data-zoom-image="{{ asset($image->image_url) }}" />
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="color-swatches">
                            ...
                            <ul>
                                @foreach($colors as $key => $color)
                                    <li>
                                        <a class="choose_{{$key}}" onclick="checkColor({{$key}})" style="background-color: {{ $color->color }}"></a>
                                        <input type="radio" class="color_{{$key}}" id="color" name="chooseColor" value="{{ $color->color_name }}" hidden />
                                    </li>
                                @endforeach
                            </ul>
                        </div>
...
@push('js')
    <script>
        function checkColor(color) {
            var $color = $('.color-swatches').find('input');
            var $choose = $('.color-swatches').find('a');
            $color.each(function () {
            ...
            $color.each(function (key) {
                var colorCurrent = $(this).val();
                if (colorCurrent !== $('.color_'+color).val()) {
                    $('.choose_'+key).css('opacity', '0.3');
                    $('.choose_'+key).css('border-color', '#000');
                } else {
                    $('.choose_'+key).css('box-shadow', '5px 5px 5px #888');
                }
            });
        }
    </script>
